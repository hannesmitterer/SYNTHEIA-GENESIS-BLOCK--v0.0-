"""
Test script for AquaLibre Protocol
"""
...
class AquaLibreProtocol:
    ...
    def process_action(self, action: Action, node_metrics=None):
        ...
        if not self.nsr_validator.validate(action):
            ...
            result["message"] = "Action violates NSR principle"
        ...
        if not self.reciprocity_filter.validate(action):
            ...
            result["message"] = "Action violates reciprocity"
            self.processing_history.append(result)
            return result
...
if __name__ == "__main__":
    ...
    # Protocol status
    print("\n📊 Protocol Status:")
    print(f"  Actions Processed: {len(protocol.processing_history)}")
    print(f"  Accepted: {sum(1 for h in protocol.processing_history if h['status'] == 'ACCEPTED')}")
    print(f"  Rejected: {sum(1 for h in protocol.processing_history if h['status'] == 'REJECTED')}")
